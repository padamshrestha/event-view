...
export class SessionComponent implements CanDeactivate, OnActivate, OnDestroy, OnInit {
  ...
  routerCanDeactivate(currTree?: RouteTree, futureTree?: RouteTree) {
    let deactivate = !this.session || !this.isDirty() || this.modalService.activate();
  ...
  private gotoSessions() { this.router.navigate(['/sessions']); }

  private handleServiceError(op: string, err: any) {
    console.error(`${op} error: ${err.message || err}`);
  }

  private isDirty() { return this.entityService.propertiesDiffer(this.session, this.editSession); }
