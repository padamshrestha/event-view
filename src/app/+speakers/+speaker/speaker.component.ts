...
import {EntityService, ModalService, Speaker, SpeakerService, ToastService} from '../../../app/shared';
@Component({
  moduleId: module.id,
  selector: 'ev-speaker',
...
export class SpeakerComponent implements CanDeactivate,
  ...
  routerCanDeactivate(currTree?: RouteTree, futureTree?: RouteTree) {
    let deactivate = !this.speaker || !this.isDirty() || this.modalService.activate();
  ...
  save() {
    ...
    this.speakerService.updateSpeaker(speaker).subscribe(
        () => this.toastService.activate(`Successfully saved ${speaker.name}`));
  }
  ...
  private gotoSpeakers() { this.router.navigate(['/speakers']); }

  private handleServiceError(op: string, err: any) {
    console.error(`${op} error: ${err.message || err}`);
  }

  private isDirty() { return this.entityService.propertiesDiffer(this.speaker, this.editSpeaker); }
